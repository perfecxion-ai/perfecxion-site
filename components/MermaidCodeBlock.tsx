  ...
  useEffect(() => {
    if (elementRef.current) {
      ...
      const renderDiagram = async () => {
        try {
          const { svg } = await mermaid.render(`mermaid-${Math.random()}`, children)
          if (elementRef.current) {
          ...
          if (elementRef.current) {
            elementRef.current.innerHTML = `
              <div class="text-red-500 p-4 border border-red-200 rounded bg-red-50 dark:bg-red-900/20">
                <div class="font-semibold mb-2">Error rendering Mermaid diagram:</div>
                <pre class="text-sm overflow-x-auto">${children}</pre>
              </div>
            `
          }
        }
