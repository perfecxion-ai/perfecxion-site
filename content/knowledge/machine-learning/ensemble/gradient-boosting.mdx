...
∂L(yᵢ,F(xᵢ)) ] 
F(xᵢ)=F 
m−1 (xᵢ) The new weak learner $h_m(x)$ is then trained to predict these pseudo-residuals. This is the core mechanism that connects the abstract optimization framework to the concrete step of training a new tree.
...
(∑g 
i ) 
2 A higher similarity score indicates that the instances in the node have more similar residuals.
...
Left +Similarity 
Right −Similarity 
Root The gain represents the improvement in the objective function resulting from the split.
