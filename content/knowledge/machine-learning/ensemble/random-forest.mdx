...
2
 ⋅n 
trees ), and its space complexity is O(N 
2
...
2
 ⋅n 
trees ))	Excellent (O(N⋅k⋅d⋅i))	Poor (O(N 
2
 logN) to O(N 
3
...
trees ⋅mtry⋅NlogN), where n 
trees is the number of trees, mtry is the number of features considered at each split, and N is the number of samples. This part is generally efficient and highly parallelizable. The second and more demanding part is the computation of the 

N×N proximity matrix. A naive pair-wise comparison has a complexity of O(N 
2
 ⋅n 
trees ). This quadratic scaling with the number of samples is the main bottleneck.
