...
description: >-
  Comprehensive guide to Naive Bayes algorithms, applications, and
  implementation.
...
P(X∣y)P(y) Each piece serves a specific purpose:
...
Number of ’Yes’ days P(Play Golf=No)=

Total number of days

Number of ’No’ days Calculate Likelihoods P(xᵢ∣y): Next, the algorithm constructs frequency or likelihood tables for each feature, conditioned on each class.
...
Count of ’Sunny’ on ’Yes’ days P(Outlook=Sunny∣Play Golf=No)=

Total count of ’No’ days

Count of ’Sunny’ on ’No’ days This process is repeated for every possible value of every feature ('Rainy', 'Overcast'; 'Hot', 'Mild', 'Cool', etc.) for both the 'Yes' and 'No' classes.
...
| Variant | Core Assumption on P(xᵢ∣y) | Data Type | Key Hyperparameters | Primary Use Case |
