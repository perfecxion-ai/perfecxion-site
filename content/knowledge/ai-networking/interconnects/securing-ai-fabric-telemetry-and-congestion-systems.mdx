...
description: >-
  Best practices for securing telemetry and congestion control mechanisms in AI
  network fabrics
...
## Practical Example: Simulating Network Jitter Impact on AI Training Duration

Below is a Python code example that models the effect of network jitter on distributed AI training job duration. The code visualizes how increasing jitter can dramatically extend total training time.

```python
import numpy as np
import matplotlib.pyplot as plt

def simulate_training_duration(base_duration_min, jitters_us):
    """
    Simulate training duration increase due to network jitter.
    Args:
        base_duration_min: Baseline training duration in minutes
        jitters_us: List of jitter values in microseconds
    Returns:
        List of total durations in minutes
    """
    # Empirical scaling: duration increases non-linearly with jitter
    durations = [base_duration_min * (1 + (jitter / 20)) for jitter in jitters_us]
    return durations

# Example usage
BASE_DURATION = 12.0  # minutes
JITTERS = [0, 20, 50, 100]  # microseconds
results = simulate_training_duration(BASE_DURATION, JITTERS)

# Visualization
plt.figure(figsize=(7, 4))
plt.plot(JITTERS, results, marker='o')
plt.xlabel('Network Jitter (µs)')
plt.ylabel('Training Duration (minutes)')
plt.title('Impact of Network Jitter on AI Training Job Duration')
plt.grid(True)
plt.tight_layout()
plt.show()
```
...
**Commentary:**
- This code models the non-linear impact of network jitter on distributed AI training duration, reflecting the sensitivity described in the article.
- You can adjust `BASE_DURATION` and `JITTERS` to match your cluster and workload.
- The visualization helps communicate why minimizing jitter is critical for reliable, efficient AI training.
