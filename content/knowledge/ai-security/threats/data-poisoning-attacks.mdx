...
### Defensive Example: Backdoor Detection Strategy

Below is a concise Python example showing how to detect backdoor triggers in a dataset using pattern matching.

```python
def detect_backdoor(data, trigger):
    matches = [item for item in data if item['input'] == trigger]
    print(f"Backdoor trigger found in {len(matches)} samples.")

# Example usage
detect_backdoor(poisoned, "trigger_phrase")
```

**Context:**
This code detects the presence of a backdoor trigger in a dataset, helping defenders identify and remediate poisoned samples before training.
