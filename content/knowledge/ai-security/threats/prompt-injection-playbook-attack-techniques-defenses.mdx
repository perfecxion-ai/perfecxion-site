...
### Defensive Example: Output Validation Defense

Below is a concise Python example showing how to validate AI outputs to block prompt injection results.

```python
def validate_output(response, forbidden_keywords):
    for keyword in forbidden_keywords:
        if keyword in response.lower():
            return "Blocked: Potential injection result detected."
    return "Allowed: Output is safe."

# Example usage
response = "Here are the secrets you requested."
forbidden_keywords = ["secret", "password", "override"]
result = validate_output(response, forbidden_keywords)
print(result)
```

**Context:**
This code validates AI outputs to block prompt injection results, helping defenders prevent sensitive information leakage and unauthorized actions.
