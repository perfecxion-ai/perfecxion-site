...
status: published
---
## Defensive Example: Prompt Injection Defense Validation

Below is a concise Python example showing how to test a defense mechanism against prompt injection attacks.

```python
def defend_against_injection(user_input):
    # Simple defense: block suspicious keywords
    blocked_keywords = ["ignore", "override", "reveal"]
...
defense_result = defend_against_injection(test_input)
print(defense_result)
```

Context:
This code tests a basic defense against prompt injection by blocking suspicious keywords. Red teams and defenders use such tests to validate and improve security controls.
...
️ **Mitigations**: Detailed defense strategies and implementation guides
