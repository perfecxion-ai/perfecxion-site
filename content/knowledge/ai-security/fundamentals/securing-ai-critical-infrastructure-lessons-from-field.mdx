...
---
### Practical Example 1: Infrastructure Anomaly Detection

Below is a concise Python example showing how to detect anomalies in critical infrastructure AI systems using simple threshold monitoring.

```python
def monitor_infrastructure(metric, threshold):
  if metric > threshold:
    print(f"Anomaly detected: metric {metric} exceeds threshold {threshold}")
  else:
    print("Infrastructure operating normally.")

# Example usage
load = 120
threshold = 100
monitor_infrastructure(load, threshold)
```

**Explanation:**
This code monitors a key infrastructure metric (e.g., grid load) and alerts when it exceeds a safe threshold, helping operators respond to potential incidents quickly.

---
### Practical Example 2: Supply Chain Integrity Check

Below is a concise Python example showing how to verify the integrity of AI supply chain components using hash validation.

```python
import hashlib

def verify_component(file_path, expected_hash):
  with open(file_path, "rb") as f:
    file_hash = hashlib.sha256(f.read()).hexdigest()
  if file_hash == expected_hash:
    print("Component integrity verified.")
  else:
    print("Component integrity check FAILED!")

# Example usage (mocked)
# verify_component("model.bin", "expectedhashvalue123...")
```

**Explanation:**
This code verifies the integrity of supply chain components (e.g., models, dependencies) using SHA-256 hashes, helping prevent tampering and supply chain attacks.
