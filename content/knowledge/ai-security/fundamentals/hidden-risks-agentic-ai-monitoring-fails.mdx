...
---
### Practical Example 1: Anomaly Detection for Agentic AI Monitoring

Below is a concise Python example showing how to use statistical anomaly detection to monitor agentic AI behavior. This helps detect unusual patterns that may indicate security risks.

```python
import numpy as np

def detect_anomaly(actions, threshold=2.0):
  mean = np.mean(actions)
  std = np.std(actions)
  for i, action in enumerate(actions):
    if abs(action - mean) > threshold * std:
      print(f"Anomaly detected at index {i}: {action}")

# Example: agent actions per hour
actions = [10, 12, 11, 13, 50, 12, 11]
detect_anomaly(actions)
```

**Explanation:**
This code uses statistical analysis to detect anomalous agent actions, which may signal unintended or malicious behavior in autonomous systems.
