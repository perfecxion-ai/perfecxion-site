...
### Malicious Example: Model Extraction Simulation

Below is a concise Python example showing how a red team might simulate model extraction by querying a model and training a substitute.

```python
def query_model(model, inputs):
  return [model(x) for x in inputs]
def train_substitute(inputs, outputs):
  # For demonstration, just pair inputs and outputs
...
def dummy_model(x):
  return x * 2
inputs = [1, 2, 3, 4]
outputs = query_model(dummy_model, inputs)
substitute = train_substitute(inputs, outputs)
print(substitute)
```

**Context:**
This code simulates a model extraction attack, where a red team queries a target model and uses the input-output pairs to train a substitute. Defenders should monitor for excessive or systematic queries.
