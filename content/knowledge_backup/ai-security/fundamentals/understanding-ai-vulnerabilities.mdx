...
Below is a concise Python example showing how an attacker might simulate model extraction by querying a model and training a substitute.
...
**Context:**
This code simulates a model extraction attack, where an attacker queries a target model and uses the input-output pairs to train a substitute. Defenders should monitor for excessive or systematic queries.
