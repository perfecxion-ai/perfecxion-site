...
---
### Practical Example 1: Agent Trust Evaluation

Below is a concise Python example showing how to evaluate trust scores for agents in a multi-agent system. This helps manage security boundaries and detect potentially risky agents.

```python
def evaluate_trust(agent_history):
  # Simple trust score: ratio of successful to total interactions
  successes = agent_history.count("success")
  total = len(agent_history)
  trust_score = successes / total if total else 0
  return trust_score

# Example agent histories
agent1_history = ["success", "success", "fail", "success"]
agent2_history = ["fail", "fail", "success", "fail"]
print(f"Agent 1 trust: {evaluate_trust(agent1_history):.2f}")
print(f"Agent 2 trust: {evaluate_trust(agent2_history):.2f}")
```

**Explanation:**
This code calculates a trust score for each agent based on their interaction history, helping security teams identify agents that may pose risks to the system.
