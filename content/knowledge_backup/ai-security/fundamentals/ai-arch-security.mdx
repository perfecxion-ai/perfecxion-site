...
---
### Practical Example: Adversarial Example Generation and Defense

Adversarial examples are inputs crafted to fool neural networks. Below is a concise PyTorch example showing how to generate an adversarial example and apply a simple defense.
...
def validate_input(data):
    # Reject inputs with abnormal values
    if torch.any(data < 0) or torch.any(data > 1):
        raise ValueError("Input out of bounds!")
    return data

# Usage Example (mocked model/data)
class DummyModel:
    def __call__(self, x):
...
validate_input(adv_data)
```

**Explanation:**
This code demonstrates how adversarial examples are generated to attack neural networks and how input validation can help defend against such attacks. In production, more advanced defenses are needed, but this illustrates the concept and risk.
