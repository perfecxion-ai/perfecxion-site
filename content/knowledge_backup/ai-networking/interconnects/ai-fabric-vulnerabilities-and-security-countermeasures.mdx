...
---
### Practical Example: Modeling All-Reduce Tail Latency Impact

```python
# Model the impact of network tail latency on distributed All-Reduce operations
# Useful for quantifying how slowest link affects AI training job completion
import numpy as np

def allreduce_tail_latency(latencies):
    # latencies: list of per-link latencies in microseconds
    tail = np.percentile(latencies, 99.9)
    print(f"99.9th percentile (tail) latency: {tail} us")
    # Estimate job completion time impact
    avg = np.mean(latencies)
    print(f"Average latency: {avg} us")
    print(f"Tail latency penalty: {tail - avg} us")

# Example usage:
latencies = np.random.normal(loc=10, scale=2, size=1000)
latencies[::100] += 20  # inject some outliers
allreduce_tail_latency(latencies)
```

*This code helps AI infrastructure teams understand how tail latency in network links can dramatically increase job completion time for distributed training. Use it to guide network upgrades and topology choices.*
