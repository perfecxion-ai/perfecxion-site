---
title: 'A Comprehensive Analysis of Density-Based Hierarchical Clustering: From DBSCAN to HDBSCAN and Beyond'
description: 'Complete guide to DBSCAN clustering, advantages, and practical applications.'
date: '2025-01-21'
...
**Complete guide to DBSCAN clustering, advantages, and practical applications**

---

## Table of Contents

- [Section 1: Fundamental Concepts](#section-1-fundamental-concepts-of-density-based-clustering)
  - [1.1 The Intuition](#11-the-intuition-defining-clusters-as-dense-regions)
  - [1.2 Core DBSCAN Algorithm](#12-core-dbscan-algorithm)
  - [1.3 Parameter Selection](#13-parameter-selection)
- [Section 2: Advanced DBSCAN Variants](#section-2-advanced-dbscan-variants)
- [Section 3: Practical Applications](#section-3-practical-applications)
- [Section 4: Performance Optimization](#section-4-performance-optimization)
- [Section 5: Comparison with Other Methods](#section-5-comparison-with-other-methods)
...
This example demonstrates DBSCAN's ability to handle irregular cluster shapes and identify noise points.
...
k (p), is defined as the distance from p to its k-th nearest neighbor.
...
k (a), will be large. The mutual reachability distance between a and any other point b will now be at least core 
k (a). This effectively "pushes" sparse points away from all other points, making sparse regions even sparser in this new transformed space.
...
1 ,C 
2 ,..., it checks the size of each new sub-cluster.
...
i in the condensed tree, it is "born" at a certain density level, λ 
birth . This is the λ value at which it split off from its parent.

The cluster "dies" when it splits into smaller clusters, at a density level λ 
death .

For each point p that belongs to cluster C 
i , we can find the λ value, λ 
p , at which it "falls out" of the cluster (or the cluster dies).

The stability of the cluster C 
i is then calculated as the sum of the persistence of each of its points:
...
2 norm) is not always appropriate. The hdbscan library supports a wide range of metrics that can be specified during instantiation :

metric='manhattan' (L 
1 norm): Useful when dimensions are not directly comparable or when outliers should have less influence.
