...
Manhattan (L1)	```math
d(p, q) = Σᵢ₌₁ⁿ |pᵢ - qᵢ|
```	Sum of absolute differences along each dimension.	When features have different scales or when you want to emphasize differences in individual dimensions.
Minkowski (Lp)	```math
d(p, q) = (Σᵢ₌₁ⁿ |pᵢ - qᵢ|ᵖ)^(1/p)
```	Generalization of Manhattan (p=1) and Euclidean (p=2) distances.	When you want to control the emphasis on larger vs. smaller differences.
Cosine Distance	```math
1 - (p·q)/(||p||·||q||)
```	Based on the angle between two vectors, ignoring magnitude.	Text analysis, document similarity, and other high-dimensional sparse data.
